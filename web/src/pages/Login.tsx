...
export default function Login() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const data = await loginHelper(api, email, password);
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          <label className="block">
            ...
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)}
              className="mt-1 w-full rounded-lg bg-slate-900 border border-slate-700 px-3 py-2 text-slate-100 focus:outline-none focus:ring-2 focus:ring-emerald-500" required />
          </label>
          <label className="block">
          ...
          <button type="submit" disabled={loading}
            className="w-full rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 text-white font-medium py-2">
